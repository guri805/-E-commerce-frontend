...
const Login = () => {
  ...
  const handleSignUpData =async (e) => {
    ...
    let response = await axios.post(`http://localhost:3001/login`,{email, password})
            ...
            {fields.map((curEle, index) => (
              <li key={curEle.id || index} className="mb-3">
                ...
                <input
                  ...
                  onChange={(e) => setLogin({ ...login, [curEle.name]: e.target.value })}
                  value={login[curEle.name]}
                />
              </li>
            ))}
          </ul>
          <button type="submit" className="btn btn-primary w-100" onClick={handleSignUpData}>
            Login
          </button>
