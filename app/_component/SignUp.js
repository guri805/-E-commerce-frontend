...
const SignUp = () => {
    const [signup, setSignUp] = useState({name:'', email:'', password:'', mobile:'', address:''});
  ...
  const handleSignUpData = async (e) =>{
    ...
    let response = await axios.post(`http://localhost:3001/register`,{name, email, password, mobile, address})
            ...
            {fields.map((curEle, index) => (
              <li key={curEle.id || index} className="mb-3">
                ...
                <input
                  ...
                  onChange={(e) => setSignUp({ ...signup, [curEle.name]: e.target.value })}
                  value={signup[curEle.name]}
                />
              </li>
            ))}
          </ul>
          <button type="submit" className="btn btn-success w-100" onClick={handleSignUpData}>
            Sign Up
          </button>
        </form>
      </div>
